...
def change_type(database, table_name, column_definitions):
    ...
    # Determine which columns need type changes
    columns_to_change = []
    for col in columns_info:
        col_name = col[1]
        col_type = col[2].upper()  # Ensure type is in uppercase for comparison
        if col_name in column_definitions and col_type != column_definitions[col_name].upper():
            columns_to_change.append(col_name)

    if not columns_to_change:
        print(f"All columns in table {table_name} already have the desired types.")
        conn.close()
        return

    # Prepare the column definitions for the new table
    new_columns = []
    for col in columns_info:
        col_name = col[1]
        if col_name in column_definitions:
            new_type = column_definitions[col_name]
            new_columns.append(f"{col_name} {new_type}")
        else:
            new_columns.append(f"{col_name} {col[2]}")

    new_columns_str = ", ".join(new_columns)

    # Create a temporary table with the new column types
    temp_table_name = f"{table_name}_temp"
    cursor.execute(f"CREATE TABLE {temp_table_name} ({new_columns_str})")

    # Copy data from the old table to the new table
    columns_only_names = ", ".join([col[1] for col in columns_info])
    cursor.execute(f"INSERT INTO {temp_table_name} ({columns_only_names}) SELECT {columns_only_names} FROM {table_name}")
...
# Definitions for table 'ehpad'
EHPAD_Indicateurs_2021_REG_agg_column_definitions = {
    ...
    'poste_reeduc': 'TEXT',
    'taux_amoxi': 'TEXT',
    'taux_antidep_2020': 'TEXT',
    'taux_antidep_2021': 'TEXT',
    'taux_antidep_2022': 'TEXT',
    'taux_anxio_2020': 'TEXT',
    'taux_anxio_2021': 'TEXT',
    'taux_anxio_2022': 'TEXT',
    'taux_atb_2021': 'TEXT',
    'taux_atb_2022': 'TEXT',
    'taux_atbr_2021': 'TEXT',
    'taux_atbr_2022': 'TEXT',
    ...
    'taux_cephalo': 'TEXT',
    'taux_deces': 'TEXT',
    'taux_demi_vie_anxio_2020': 'TEXT',
    'taux_demi_vie_anxio_2021': 'TEXT',
    'taux_demi_vie_anxio_2022': 'TEXT',
    'taux_fluoro': 'TEXT',
    'taux_hospit_had': 'TEXT',
    'taux_hospit_mco': 'TEXT',
    'taux_hypno_2020': 'TEXT',
    'taux_hypno_2021': 'TEXT',
    'taux_hypno_2022': 'TEXT',
    'taux_neuro_2020': 'TEXT',
    'taux_neuro_2021': 'TEXT',
    'taux_neuro_2022': 'TEXT',
    'taux_neuro_alzh_2020': 'TEXT',
    'taux_neuro_alzh_2021': 'TEXT',
    'taux_neuro_alzh_2022': 'TEXT',
...
# Definitions for table 'export_tdb_esms_2022'
export_tdbesms_2022_region_agg_column_definitions = {
    "index" : "INTEGER",
    "region" : "TEXT",
    "departement" : "TEXT",
    "categorie" : "TEXT",
...
# Definitions for table 'export_tdb_esms_2022'
commune_2022_region_agg_column_definitions = {
    'index' : 'INTEGER',
    'typecom': 'TEXT',
    'com' : 'TEXT',
    'reg' : 'REAL',
...
region_2022_column_definitions = {
    'index': 'INTEGER',
    'reg': 'INTEGER',
    'cheflieu': 'TEXT',
    'tncc': 'INTEGER',
